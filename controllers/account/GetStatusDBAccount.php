<?php
    ...
    if($_GET['pageIndex']  && $_GET['perPage']  ){
        // selectStatus = 1 la tat ca 
        // selectStatus = 0 là đang hoạt đôngk
        // selectStatus = -1 đã khóa 
        if($_GET['selectStatus'] == 1 ){
            $sql = "SELECT * FROM account";
            $sqlCount = "SELECT COUNT(*) as TotalRow FROM account WHERE (status = 0 or status = 1)";
            ...
            $sqlPagination = "SELECT * FROM account WHERE (status = 0 or status = 1) ORDER BY id DESC  LIMIT ".$begin." , ".$perPage;
            ...
            $sql = "SELECT * FROM account";
            $sqlCount = "SELECT COUNT(*) as TotalRow FROM account WHERE status = 1";
            $result = $conn->query($sql);
            $resultTotalRow = $conn->query($sqlCount);
            $TotalRow = 0;
            foreach($resultTotalRow as $value){
            ...
            $perPage = $_GET['perPage'];
            $begin = ($pageIndex * $perPage) - $perPage;
            $sqlPagination = "SELECT * FROM account WHERE status = 1 ORDER BY id DESC  LIMIT ".$begin." , ".$perPage;
            $resultPagination = $conn->query($sqlPagination);
            $accounts = array();
            foreach($resultPagination as $valuePagination){
                $account = array(
